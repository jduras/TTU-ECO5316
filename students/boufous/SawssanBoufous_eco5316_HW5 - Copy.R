...
TNPE_tbl_all %>%
  gather(variable, value, -yearm, -date) %>%
...
data.ts.1 <- TNPE_tbl_all %>%
  tk_ts(select = ly, start = train.start.n, end = test.start.n , frequency = 12)
...
results <-
  ...
  mutate(sarima.model = slide(ly, ~Arima(.x, order = c(3,1,0), seasonal = list(order = c(1,1,1), period = 12)),.size = window.length)) %>%            
...
data.ts.1<- TNPE_tbl_all%>%
  filter(yearm <= yearmonth(lstm1)) %>% filter(!is.na(dy))%>%
  tk_ts(select = dy, end=lstm3, frequency = 12)

m3<- data.ts.1 %>%
  tk_ts(select = dy, end=lstm3, frequency = 12) %>%
  auto.arima(ic = "aic",  seasonal = TRUE,stationary = TRUE, stepwise = FALSE, approximation = FALSE)
m3
ggtsdiag(m3)
# construct 1-month ahead forecasts
hmax <- 150
f00<- forecast(m3, hmax) 
f00 
...
# Recode of actual data   
act_tbl_22 <- 
  TNPE_tbl_all %>% 
  mutate(index <= as.yearmon(lstm2)) %>%
  select(index, dy) %>% 
  mutate(key = "actual", 
         date = as.Date(index)) %>% 
  select(date, key, dy) %>% filter(year(date) >= 2008) 
act_tbl_22
# Recode the multistep forecasts, convert to levels 
TNPE_tbl_f_22 <- f00 %>% 
  sw_sweep() %>%  
  filter(key == "forecast") %>% 
  mutate_at(vars(dy, lo.80, lo.95, hi.80, hi.95), funs()) %>%   
  ...
  select(date, key, dy, lo.80, lo.95, hi.80, hi.95) 
# Actual and Forecast Data combined
fc1_act_data_tbl <- bind_rows(act_tbl_22,  TNPE_tbl_f_22,.id = NULL) 
fc1_act_data_tbl 


# plot 1-month ahead rolling forecasts - levels
ppf<-fc1_act_data_tbl %>%
...
#(k) 
accuracy(TNPE_tbl_f_2$y, act_tbl_2$y)
#the forecast errors for $\Delta E_t$
accuracy(TNPE_tbl_f_2$dy, act_tbl_2$dy)
