...
gf1
```
Comments: # from the result, this forecast perform well. The trend and seasonal pattern of the forecasts are close to the characteristics of the actual data.  
...
gf2
```
Comments: from the result, this forecast doesn't perform well. To be more specific, the forecasts are far away from the actual data. 
...
f_a_tbl_2 %>% 
  ...
  spread(key,dE)%>%
  mutate(error=actual-forecast,
         percent_error=(actual-forecast)/actual*100) %>%
  tk_ts(select=percent_error, start=2008,frequency = 12)%>%
  autoplot()
```
Comments: From the graphs we can get the forecast percentage error term for $E_t$ vibrates from -1.5% to 1%, since the unit for the data is "thousands of person", so the error term is changing from 15 people to 10 people, which implies the forecasts for $E_t$ are good. However forecast percentage error term for $\Delta E_t$ from November 2015 to the end of 2015,  increases from 0 to 5000000%, and then go back to 0, which means the forecasts deviates too far away from the actual data for this short period; in other periods, the error term changes approximately from -100000% to 100000%, considering the unit for the data, the range is (-1000000,1000000), therefore we can conclude that the forecasts for the Delta E_t are not good. All the above conclusions are consistent with the results in (i) and (j).